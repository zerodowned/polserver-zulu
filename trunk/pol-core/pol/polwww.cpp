...
bool http_readline( Socket& sck, std::string& s, int length = -1 )
{
	//cout << "{+RL}";
	bool res = false;
	s = "";
...
bool start_http_script( Socket& sck, 
						...
						const string& query_string, const string& x_forwarded_for, const string& post_data )
{
	bool res = true;
	...
	else
	{
		...
		if(!post_data.empty()) {
			hem->read_query_string( post_data );
		}
		hem->read_query_ip();

		if( !x_forwarded_for.empty() ) {
			hem->query_ip_ = x_forwarded_for; 
		}
...
void http_func( SOCKET client_socket )
{
	...
	while (sck.connected() && http_readline( sck, tmpstr, post_data_starts?content_len:-1 ))
	{
		if (config.web_server_debug)
			cout << "http(" << sck.handle() << "): '" << tmpstr << "'" << endl;

		if (tmpstr.empty()) {			
			if(!post.empty() && !content_type.empty() && content_len!=-1) {
				post_data_starts = true;
				continue;
			}			
			break;
		}
		
		if(post_data_starts==true) {
			post_data = tmpstr;
			break;
		}
		
		if(strnicmp( tmpstr.c_str(), "Content-Type", 12 ) == 0) {
			//TODO: reject unsupported content encodings
			content_type = tmpstr.substr( 13 );
		}

		if(strnicmp( tmpstr.c_str(), "Content-Length", 14 ) == 0) {
			char* end;
			content_len = strtol(tmpstr.substr( 15 ).c_str(), &end, 10);

			if (!*end) {
				//printf("Converted successfully\n");
			}
			else {
				cout << "Warning: wrong/spoofed Content-Length header supplied!";
				content_len = -1;
			}
		}
		...
		else if (strnicmp( tmpstr.c_str(), "post", 4 ) == 0) {
			post = tmpstr;
		}

		if (strncmp( tmpstr.c_str(), "Authorization:", 14 ) == 0)
			auth = tmpstr;
		if (strnicmp( tmpstr.c_str(), "Host: ", 5 ) == 0)
			host = tmpstr.substr( 6 );
		
		// TODO: add maybe more headers such as X-Real-Ip
		if (strnicmp( tmpstr.c_str(), "X-Forwarded-For: ", 17 ) == 0) {
			// TODO: handle multiple addresses
			//if(tmpstr.find(",")!=std::string::npos) {
			//}
			x_forwarded_for = tmpstr.substr( 17 );
		}
	}
	if (!sck.connected())
		return;	
...
//		url = "/index.htm";
	...
	if (pagetype == "ecl")
	{
		...
		start_http_script( sck, page, pkg, filename, 
			query_string, x_forwarded_for, post_data );
	}
