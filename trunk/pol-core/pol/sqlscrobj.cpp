...
BObjectRef BSQLRow::OperSubscript(const BObject& obj) {
	...
	if (right.isa( OTLong )) // vector
	{
		...
		utf8tombs(_row[index-1], decoded);				
		return BObjectRef( new String((const char*)&decoded[0]) );
#else
...
bool BSQLConnection::query(const char *query) {
	...
	std::vector<char> utf8Query;
	// endcode to utf-8
	mbtoutf8s(query, utf8Query);

	if (mysql_query(_conn, &utf8Query[0]))
#else	
