...
BObjectImp* BPacket::call_method_id( const int id, Executor& ex, bool forcebuiltin )
{
	...
	case MTH_GETSTRINGZLIBBED:
		{
			if(ex.numParams() != 2) {
			    return new BError( "GetStringZlibbed requires 2 parameters." );
			}
               ...
               ex.getParam( 1, len ))
            {
				...
				unsigned long uncompressed_len = len * 4;
				Bytef* dec_string = new Bytef[uncompressed_len];

				const Bytef* source = reinterpret_cast<const Bytef*>(&buffer[offset]);

				int uncompr_result = uncompress(dec_string, &uncompressed_len, 
					source, len);

				if(uncompr_result==Z_OK) {
					//int real_len = 0;
					const char* str_offset = reinterpret_cast<const char*>(dec_string);
					// Returns maximum of len characters or up to the first null-byte
					//while (real_len < uncompressed_len && *(str_offset+real_len))
					//	real_len++;
					String* res = new String(str_offset, uncompressed_len);
					delete[] dec_string;
					return res;
				} else 
				if (uncompr_result==Z_MEM_ERROR){
					delete[] dec_string;
					return new BError( "Out of memory" );
				} else 
				if (uncompr_result==Z_BUF_ERROR){
					delete[] dec_string;
					return new BError( "Buffer is too small" );
				} else 
				if (uncompr_result==Z_DATA_ERROR){
					delete[] dec_string;
					return new BError( "Bad compressed data" );
				}

				delete[] dec_string;
				return new BError( "Unknown uncompression error" );
            }
			else {
                return new BError( "Invalid parameter" );
			}
