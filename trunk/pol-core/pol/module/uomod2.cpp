...
const char RUS_ASCII_START = 'À';
const char RUS_ASCII_END = 'ÿ';
...
BObjectImp* UOExecutorModule::internal_SendUnCompressedGumpMenu(Character* chr, ObjArray* layout_arr, ObjArray* data_arr, long x,long y)
{
	...
	*addpt++ = '\0';
	...
	for( unsigned i = 0; i < data_arr->ref_arr.size(); ++i )
	{
		...
		wchar_t *wszToCopy = (wchar_t*) &pdata->text[0];

		MultiByteToWideChar( CP_ACP, 0, string, -1, wszToCopy, textlen ); 

		while (*wszToCopy)
		{	
			*wszToCopy = htons(*wszToCopy++);
		}
#else
		unsigned short current_data_s;
		unsigned short* tt = (unsigned short*)&pdata->text[0];

		while (*string)				
		{			
			if( (*string) >= RUS_ASCII_START && (*string) <= RUS_ASCII_END) {
				current_data_s = *string + 1040 - RUS_ASCII_START;
			} else {
				current_data_s = *string;
			}
			
			*tt++ = htons(current_data_s);
			++string;
		}
#endif
...
void gumpbutton_handler( Client* client, PKTIN_B1* msg )
{
	...
	else
	{
		...
		for( unsigned i = 0; i < strings_count; ++i )
		{
			...
			wchar_t* w_str = (wchar_t*) &strentry->data[0];							
			std::vector<char> mbTextEntryStr;

			for( int si = 0; si < length; ++si ) {
				w_str[si] = ntohs( w_str[si] );
			}
					
			utf16tombs(w_str, mbTextEntryStr, length, ".1251");

			str.append( (char*) &mbTextEntryStr[0] );
