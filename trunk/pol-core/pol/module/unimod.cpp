...
BObjectImp* UnicodeExecutorModule::mf_SendSysMessageUC()
{
	...
	BObjectImp* msg_text_imp = getParamImp( 1 );
        ...
        getParam( 4, color ))
    {
        if (chr->has_active_client())
        {
			if( msg_text_imp->isa(BObjectImp::OTArray) ) {

				oText =  static_cast<ObjArray*>(msg_text_imp);		
				unsigned textlen = oText->ref_arr.size();
				if ( textlen > SPEECH_MAX_LEN )
					return new BError( "Unicode array exceeds maximum size." );
			...
			} else if(msg_text_imp->isa(BObjectImp::OTString)) {
				sText = static_cast<String*>(msg_text_imp);		
				std::string std_s_text = sText->getStringRep();
	
				if ( std_s_text.length() > SPEECH_MAX_LEN )
					return new BError( "String exceeds maximum size." );
				
				gwtext[std_s_text.length()] = L'\0';

				MultiByteToWideChar( CP_ACP, 0, std_s_text.c_str(), -1, (wchar_t*)&gwtext[0], std_s_text.length());
			} else {
