...
std::hash_map<std::string, std::string> UOExecutorModule::super_cache;

BObjectImp* UOExecutorModule::mf_CacheObject()
{
	//UObject* uobj;
    const String* keyname_str;
	const String* value;
    if (getStringParam( 0, keyname_str ) && getStringParam( 1, value ))
    {
		UOExecutorModule::super_cache[keyname_str->getStringRep()] = value->getStringRep();
        return new BLong(1);
    }
    else
    {
...
BObjectImp* UOExecutorModule::mf_GetCachedObject()
{
	//UObject* uobj;
    const String* keyname_str;
    if (getStringParam( 0, keyname_str ))
    {
		std::string result;	
		std::string key = keyname_str->getStringRep();

		UOExecutorModule::ImpCache::iterator it = UOExecutorModule::super_cache.find(key);
		
		//propval->ref
		//BObjectImp* cached_value = ;
		if( it == UOExecutorModule::super_cache.end() ) {
			return new BError( "Key not found" );
		}
        ...
        chr->client->gd->target_cursor_uoemod != NULL)
    {
        if (msg != NULL)
        {
			int x = cfBEu16(msg->x),
				y = cfBEu16(msg->y),
				z = msg->z,
				graphic = cfBEu16(msg->graphic);

			Realm* realm = chr->realm;

			if (!realm->valid(x, y, static_cast<short>(z))) {
				chr->client->gd->target_cursor_uoemod->uoexec.ValueStack.top().set( new BObject(new BError("Invalid Coordinates for realm") ) );
				goto endpacket;
			}

            BStruct* targetDescr = new BStruct;
            targetDescr->addMember( "x", new BLong( x ) );
            targetDescr->addMember( "y", new BLong( y ) );
            targetDescr->addMember( "z", new BLong( z ) );		
			targetDescr->addMember( "realm", new String( realm->name() ) );
...
//			target on other realms. DUH!
            ...
            for( unsigned i = 0; i < slist.size(); ++i )
            {
				...
				if(slist[i].z == z) {
					if( slist[i].objtype == graphic ) {
						targetDescr->addMember( "objtype", new BLong( slist[i].objtype ) );
						//targetDescr->addMember( "hue", new BLong( slist[i].hue ) );
						//found = true;
						break;
					}
				}
            ...
            if (selected_serial)
            {
                ...
                if (obj)
                {
                    targetDescr->addMember( obj->target_tag(), obj->make_ref() );
					targetDescr->addMember( "objtype", new BLong( obj->objtype_ ) );
                }
            }

			UMulti* multi = chr->realm->find_supporting_multi( cfBEu16(msg->x), cfBEu16(msg->y), msg->z );
			if ( multi != NULL ) {
				targetDescr->addMember( "multi", multi->make_ref() );
			}
			
			if(targetDescr->contents().count( "objtype" ) == 0)
			{
				targetDescr->addMember( "objtype", new BLong( 0 ) );
			}
			

            chr->client->gd->target_cursor_uoemod->uoexec.ValueStack.top().set( new BObject( targetDescr ) );
        }
...
BObjectImp* UOExecutorModule::mf_ListItemsNearLocation(/* x, y, z, range, realm */)
{
		...
		getStringParam( 4, strrealm) )
    {
		...
		//#pragma omp parallel for private(wx, wy, itr) schedule(dynamic) num_threads(8)
...
void UOExecutorModule::internal_InBoxAreaChecks(unsigned short &x1, 
	unsigned short &y1, long &z1, unsigned short &x2, unsigned short &y2, long &z2, Realm* realm)
{
	if ( (z1 < WORLD_MIN_Z) || (z1 == LIST_IGNORE_Z) )
		z1 = WORLD_MIN_Z;
	...
	if ( (z2 > WORLD_MAX_Z) || (z2 == LIST_IGNORE_Z) )
		z2 = WORLD_MAX_Z;
}
        ...
        for( unsigned short wy = wyL; wy <= wyH; ++wy )
        {
            ...
            for( ZoneCharacters::iterator itr = wchr.begin(), end = wchr.end(); itr != end; ++itr )
            {
                ...
                if (chr->x >= x1 && chr->x <= x2 &&
                    chr->y >= y1 && chr->y <= y2 &&
                    ((chr->z >= z1) || (z1 == LIST_IGNORE_Z)) && (chr->z <= z2 || (z2 == LIST_IGNORE_Z)))
                {
        ...
        for( unsigned short wy = wyL; wy <= wyH; ++wy )
        {
            ...
            for( ZoneItems::iterator itr = witem.begin(), end = witem.end(); itr != end; ++itr )
            {
                ...
                if (item->x >= x1 && item->x <= x2 &&
                    item->y >= y1 && item->y <= y2 &&
                    (item->z >= z1 || (z1==LIST_IGNORE_Z)) && (item->z <= z2 || (z2==LIST_IGNORE_Z)))
                {
                    newarr->addElement( item->make_ref() );
                }
            }
                    ...
                    if ( (inc_hidden && chr->hidden()) || (inc_dead && chr->dead()) || (inc_concealed && chr->concealed()) || (inc_normal && !(chr->hidden()||chr->dead()||chr->concealed())) )
					{
						if ((abs(chr->x - x) <= range) && (abs(chr->y - y) <= range))
						{
							if ((z == LIST_IGNORE_Z) || (abs(chr->z - z) < CONST_DEFAULT_ZRANGE))
							{
								...
								else if (inc_players_only && !chr->script_isa(POLCLASS_NPC)) {
									//newarr->addElement( chr->make_ref() );
									if (chr->client->ready) {
										newarr->addElement( chr->make_ref() );
									}
...
BObjectImp* UOExecutorModule::mf_GetMidpointCircleCoords()
{

#define APPEND_POINT(X,Y) {\
	point = new BStruct; \
	...
	if ( !(getParam(0, x0) && getParam(1, y0)
		&& getParam(2, radius)) )
	{
		return new BError("Invalid parameter type");
	}
	...
	APPEND_POINT(x0, y0 + radius)
	APPEND_POINT(x0, y0 - radius)
	APPEND_POINT(x0 + radius, y0)
	APPEND_POINT(x0 + radius, y0)
	...
	while(x < y) {
        ...
        ddF_x += 2;
        f += ddF_x;    
        APPEND_POINT(x0 + x, y0 + y)
        APPEND_POINT(x0 - x, y0 + y)
        APPEND_POINT(x0 + x, y0 - y)
        APPEND_POINT(x0 - x, y0 - y)
        APPEND_POINT(x0 + y, y0 + x)
        APPEND_POINT(x0 - y, y0 + x)
        APPEND_POINT(x0 + y, y0 - x)
        APPEND_POINT(x0 - y, y0 - x)
	}
...
BObjectImp* UOExecutorModule::mf_GetSubTileCoord()
{
	double x, y, z;
	...
	const double ltrs = 0.24;
	const double htrs = 0.76;

	double xfrac = x - floorf(x); 
	double yfrac = y - floorf(y);
	...
	if( xcheks ) {
		if(	yfrac < ltrs ) {
			...
			result->addMember("x", new BLong(ceilf(x)));
			result->addMember("y", new BLong(ceilf(y)));
			result->addMember("z", new BLong(int(z)+6));
			result->addMember("adjacent", new BLong(1));
			return result;
		}
	} else if(yfrac > 0.5)
		y = ceilf(y);
	...
	result->addMember("x", new BLong(int(x)));
	result->addMember("y", new BLong(int(y)));
	result->addMember("z", new BLong(int(z)));
	result->addMember("adjacent", new BLong(0));
	return result;
}
...
UOFunctionDef UOExecutorModule::function_table[] =
{
	...
	//{ "CacheObject",			&UOExecutorModule::mf_CacheObject },
	//{ "GetCachedObject",		&UOExecutorModule::mf_GetCachedObject },
	{ "GetSubTileCoord",		&UOExecutorModule::mf_GetSubTileCoord },
